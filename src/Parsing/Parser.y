...
%type <StatementVariant> inner_stmt
...
assign_stmt:
    IDENT ASSIGN value SEMICOLON {
      $$ = AssignmentStatement{
          std::move($1),
          std::make_unique<ValueVariant>(std::move($3))};
    }
;

print_stmt:
    PRINT_KW LEFT_PAREN value RIGHT_PAREN SEMICOLON {
      $$ = PrintStatement{
          std::make_unique<ValueVariant>(std::move($3))};
    }
;
...
bool_expr:
    value comp_op value {
      $$ = BoolExpression{
          std::make_unique<ValueVariant>(std::move($1)),
...
value:
    IDENT {
      $$ = ValueVariant{IdentifierExpression{std::move($1)}};
    }
  | NUM {
      $$ = ValueVariant{NumberExpression{$1}};
    }
;
